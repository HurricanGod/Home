...
+ 是一个无序的字符串集合，添加、删除、测试元素是否存在的复杂度都为`O(1)`
+ 可以高效地进行并集、交集、差集操作
+ 集合对象的底层编码可以有两种：`intset`、`hashtable`
  + `hashtable`编码的集合对象使用字典作为底层实现，字典每一个元素是一个字符串对象，每个字符串对象是集合中的一个元素，字典的值为null

------

<a name="orderset">**有序集合对象**</a>

+ 有序集合的编码可以是`ziplist`或`skiplist`
  + `ziplist`编码的有序集合使用**压缩列表**作为底层实现，**压缩列表**内的元素按照`score`从小到大进行排序
  + `skiplist`编码的有序集合对象使用`zset`结构作为底层实现，一个`zset`结构同时包含一个字典和跳跃表
+ 有序集合中的每个元素成员都是一个字符串对象，每个元素的分值都是1个double类型的浮点数
...
**有序集合同时使用跳跃表和字典来实现的原因**：

+ 如果**只使用跳跃表**来实现有序集合，执行范围型的操作优点保留，根据成员查找分值的操作会使用复杂度从`O(1)`上升到`O(log2n)`
+ 如果**只使用字典**来实现有序集合，字典以`O(1)`复杂度查找成员对应的`score`的优势保留，但字典是无序存放元素的，要保证有序性需要进行排序，排序最小的时间开销为`O(log2n)`，另外还需要`O(n)`的空间复杂度
